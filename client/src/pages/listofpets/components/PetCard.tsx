...
interface PetCardProps {
  petDetails: Pet;
  setTogglePetConditions: (value: { toggle: boolean; data: Pet }) => void;
  favouritedPets?: Pet[];
  reservedPets?: Pet[];
}
...
export default function PetCard({
  ...
  useEffect(() => {
    const isAlreadyFavourite = favouritedPets.some(
      (favPet) => favPet.pet_id === petDetails.pet_id
    );
  ...
  useEffect(() => {
    const isPetReserved = reservedPets.some(
      (reservedPet) => reservedPet.pet_id === petDetails.pet_id
    );
  ...
  const reservePet = async () => {
    ...
    const user = userSession ? JSON.parse(userSession) : null;
    if (!user) {
      alert("You need to log in to reserve pets.");
      return;
    }
    ...
    try {
      ...
      console.error("Error reserving pet:", error);
      alert("An error occurred while reserving the pet.");
    }
  };
  ...
  return (
    <article className="w-96 h-full border-2 rounded-lg shadow-xl mb-4">
      ...
      <div className="flex flex-col h-3/6 justify-evenly p-4 tracking-wide overflow-y-auto overflow-x-hidden break-words">
        ...
        <div className="flex flex-row mb-2">
          <p className="text-2xl italic underline font-bold mr-1">{petDetails.name}</p>
        </div>
        <div className="flex flex-row mb-2">
      ...
      <div className="flex flex-row h-1/6 justify-between items-center space-x-4 p-4 border-t-2">
        <button
          ...
          onClick={() => setTogglePetConditions({ toggle: true, data: petDetails })}
        >
          View Pet Conditions
        </button>
